...
//let numGroups = 6 //分成4组的话
let numTopics = 128
let numStu = 128
let groupMentors = function(numGroups) { //根据导师的类别比例进行分组
    
    db.mentors.count({}).exec() //numMentors是所有导师的数量
        .then((numMentors) => {
            let numGroupMembers = 20
            numGroupMembers = Math.floor(numMentors / numGroups) //向下取整,每组中有多少人
            let leftMentors = numMentors - numGroupMembers * numGroups //会剩下这么多人没得分组
            let query = db.mentors.find()
            ...
            query.exec()
                .then((mentors) => {
                    //console.log(mentors)
                    let groupId = 0
                    let newNumGroupMembers = numGroupMembers
                    for (let i = 0; groupId + 1 <= numGroups && i <= numMentors - numGroupMembers; i += numGroupMembers) {
                        let group = {
                            _id: ++groupId,
                            mentors: [],
                            fields: []
                        ...
                        if (numGroups - groupId + 1 == leftMentors) //到了最后面几组，一组给一个分不完的人
                            newNumGroupMembers++
                            for (let j = 0; j < newNumGroupMembers; j++) {
                                ...
                                db.mentors.findOneAndUpdate({ _id: mentors[i + j]._id }, { $set: { group: group._id } }, { new: true }).exec()
                                    ...
                                    // mentors[i+j].save()
                                for (let k = 0; k < mentors[i + j].fields.length; k++) {
                        ...
                        if (newNumGroupMembers > numGroupMembers) i++
                            ...
                            let myGroup = new db.groups(group)
                        myGroup.save()
                    }
                    //console.log(group)
...
let groupTopics = function(numGroups) {
    db.topics.find({}, ['_id', 'mentor', 'fields', 'finalstudents']).exec()
        .then((topics) => {
            let numStudentsGroup = 5
            db.groups.find({}).exec()
                .then((groups) => {
                    let recordGroup = []
                    let recordTopic = []

                    for (let n = 0; n < groups.length; n++) recordGroup.push({ numStudents: 0 })
                    for (let i = 0; i < topics.length; i++) {
                        recordTopic.push({ isgrouped: false })
                        for (let j = 0; j < groups.length; j++) { //如果这组里有这个题目的导师
                            if (groups[j].mentors.indexOf(topics[i].mentor) != -1 || recordGroup[j].numStudents > numStudentsGroup) //如果题目的老师在这组里,就将题目放下一组
                                continue
                            for (let k = 0; k < topics[i].fields.length; k++)
                                if (groups[j].fields.indexOf(topics[i].fields[k]) != -1) { //如果这组有题目的研究方向
                                    ...
                                    for (let s = 0; s < topics[i].finalstudents.length; s++) {
...
let finalgroup = function(numGroups){
