...
let firstGroup = function(data, n) {
    ...
    for (var i = 0; i < numMentors; i++) {
        var mentor = {}
        mentor.dist = data[i][0]//距离矩阵
        mentor.name = data[i][1]//单个id
    ...
    _.forEach(groups, (_g, i) => {
        ...
        new Promise((resolve,reject)=>{
           _.forEach( _g.mentors, (mentor) => {
            db.mentors.findOneAndUpdate({ _id: mentor }, { $set: { group: group._id } }, { new: true }).exec() //把每位导师的组 设置为对应的组别
            db.mentors.find({_id:mentor},(err,mf)=>{
             for (let k = 0; k < mf[0].fields.length; k++) {
                if (group.fields.indexOf( mf[0].fields[k]) === -1) //有返回下标,没有返回-1
                    group.fields.push( mf[0].fields[k]) //如果没有的话才导入所在组的方向
            }
            })
            .then(()=>{
              resolve(group)
...
var groupTopics = function(numGroups) {
        db.topics.find({}, ['_id', 'mentor', 'nfields', 'finalstudents']).exec()
            .then((topics) => {
                ...
                db.groups.find({}).exec()
                    .then((groups) => {
                        ...
                        for (var n = 0, nlen = groups.length; n < nlen; n++){
                           recordGroup.push({ numStudents: 0 })//每组里有0个学生作为初始的
                        }
                        ...
                        for (var i = 0, ilen = topics.length; i < ilen; i++) {
                            ...
                            for (var j = 0, jlen = groups.length; j < jlen; j++) { //如果这组里有这个题目的导师
                                if (groups[j].mentors.indexOf(topics[i].mentor) !== -1 || recordGroup[j].numStudents > numStudentsGroup) //如果题目的老师在这组里,就将题目放下一组
                                    continue
                                for (var k = 0, klen = topics[i].nfields.length; k < klen; k++)
                                    if (groups[j].fields.indexOf(topics[i].nfields[k]) != -1) { //如果这组有题目的研究方向
